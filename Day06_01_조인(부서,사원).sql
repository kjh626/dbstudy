...
-- 외래키 제약조건이 없는 상태이므로, 제약조건을 위배하는 데이터를 입력할 수 있다. DEPART 5는 NULL로 나온다.
...
DELETE FROM EMPLOYEE_TBL WHERE EMP_NO = 1005;   -- PK를 조건으로 사용하면 인덱스를 타기 때문에 빠르다.(DB에서 중요한 것은 찾는 속도다. 찾는 속도가 성능에 영향을 미친다.)
