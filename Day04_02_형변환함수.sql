...
-- '100' -> 100
SELECT '100', TO_NUMBER('100')   -- 문자는 왼쪽정렬, 숫자는 오른쪽정렬돼서 보여짐.
  FROM DUAL;
...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 WHERE EMPLOYEE_ID = '150';     -- EMPLOYEE_ID는 숫자, '150'은 문자. EMPLOYEE_ID = TO_NUMBER('150')으로 자동으로 변환한 뒤 처리한다.
...
SELECT 
     ...
     , TO_CHAR(100, '000000')   -- '000100'  -- 0은 0을 채우고, 100은 100을 채운것
     , TO_CHAR(1234, '9,999')   -- '1,234'
     , TO_CHAR(12345, '9,999')  -- ###### (5자리 12345를 4자리 형식 9,999에 맞출 수 없다.)
     , TO_CHAR(12345, '99,999') -- '12,345'
     , TO_CHAR(0.123, '0.00')   -- '0.12' (반올림, 소수 2자리 
...
-- 2) ★★★날짜를 문자로 변환하기(중요)★★★
-- 날짜를 원하는 형싱으로 변환할 때 사용한다.
-- '2023-02-07' -> '2023년 02월 07일'

-- 먼저, 현재 날짜와 시간 확인하기
SELECT 
       SYSDATE       -- DATE 타입의 현재 날짜와 시간(조회하면 시간이 나오진 않음)
...
/* 3. 날짜로 변환하기(TO_DATE) */
-- 날짜와 시간을 원하는 형식으로 표시하는 함수가 아님을 주의하십시오. -> TO_CHAR로 쓰는 거임.
...
-- EMPLOYEES 테이블에서 2000/01/01 ~ 2005/12/31 사이에 입사한 사원 조회
