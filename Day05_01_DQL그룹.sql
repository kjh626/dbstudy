...
-- DISTINCT 없어도 할 수 있다. 중복제거하듯이 모인다. 그리고 GROUP BY절은 중복된 데이터가 몇 개인지 알아낼 수 있다. => 함수를 쓸 수 있다.
...
-- 참고. GROUP BY 없이 집계함수 사용하기 (크게 신경 안 써도 된다)
SELECT 
...
/*
    ...
    3. GROUP BY 이후에만 처리할 수 있는 조건은 HAVING절이 처리한다.
      -- 그룹을 안 해도(나머지 모두) 사용할 수 있는 조건 -> WHERE
           -- 예시) 부서별 인원수가 5명 이하인 부서를 조회
      -- 그룹바이를 해야만 사용할 수 있는 조건 -> HAVING
*/

-- 5. DEPARTMENT_ID가 NULL인 부서를 제외하고, 모든 부서별 사원 수를 조회하시오.
