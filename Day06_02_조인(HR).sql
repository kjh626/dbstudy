...
-- 3. 모든 사원들의 EMPLOYEE_ID, FIRST_NAME, DEPARTMENT_NAME, CITY를 조회하시오. (테이블이 3개 사용됨)
-- LOCATIONS - DEPARTMENTS - EMPLOYEES // L과 D 를 먼저하면 둘이 합쳐짐. 그 후에 합친LD와 E를 조인하면 됨. 그래야 PK가 먼저나오고 FK가 나오는 순서가 나온다.
...
-- 2) 오라클 문법
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, D.DEPARTMENT_NAME, L.CITY
  FROM LOCATIONS L, DEPARTMENTS D, EMPLOYEES E      -- 1(L) : M(D) , 1(D) : M(E) 이렇게 1대다 관계
...
SELECT D.DEPARTMENT_NAME AS 부서명
  ...
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME; -- 보고싶으면? SELECT문에서 조회하고자 하는 칼럼은 반드시 GROUP BY에도 적어줘야한다. 똑같이 적어줄 필요는 없음. GROUP_BY에 DEPARTMENT_ID 적어줘도 규칙 잘 지켰음 -> 잘 돌아감

-- 부서별 DEPARTMENT_ID과 사원 수와 평균 연봉을 조회하시오 (조인 없이 가능)
