...
*/
SELECT ROW_NUMBER() OVER(ORDER BY SAL DESC) AS 연봉순위, P.PNAME AS 교수명, P.SAL AS 연봉
  FROM PROFESSOR P;
...
RETURN NUMBER
IS 
    NO STUDENT.STUDNO%TYPE;
BEGIN
    SELECT STUDNO
      INTO NO   -- NAME은 EMPNAME에 저장한다.
      FROM STUDENT
     WHERE USERID = ID;
    RETURN NO;
END;
...
IS
PROF_NO DEPARTMENT.DEPTNO%TYPE;
BEGIN
    SELECT PROFNO
      INTO PROF_NO
      FROM DEPARTMENT D
    ...
    DELETE 
      FROM STUDENT S
     WHERE S.DEPTNO = DEPT_NO;
    DELETE
      FROM DEPARTMENT D
     WHERE D.DEPTNO = DEPT_NO;
    DELETE
      FROM PROFESSOR P
     WHERE P.PROFNO = PROF_NO;
    COMMIT;
